...
window.startDay=async function(){
  ...
  // Afficher la liste des patients EN PREMIER (renderLivePatientList écrit
  // dans #uber-next-patient — le même élément que _renderNextPatient).
  renderLivePatientList();

  // PUIS publier nextPatient → déclenche _renderNextPatient (listener APP.on)
  // qui écrit la card du prochain patient PAR-DESSUS la liste, donc visible
  // immédiatement. Sans ça, la liste écrasait la card et l'utilisateur voyait
  // la liste pendant ~5s avant que le throttle GPS ne ré-affiche la card.
  if (firstP && typeof APP.set === 'function') APP.set('nextPatient', firstP);
