...
async function markUberDone() {
  ...
  /* ⚡ ORDRE CRITIQUE : la liste AVANT la card prochain patient.
     `_updateUberProgress()` appelle `renderLivePatientList()` qui écrit la
     liste complète des patients dans `#uber-next-patient`. Or `selectBestPatient()`
     déclenche (via APP.set('nextPatient')) `_renderNextPatient()` qui écrit
     la card prochain patient dans le MÊME élément.
     Si on appelle dans l'ordre inverse (selectBestPatient puis _updateUberProgress),
     la liste écrase la card et l'utilisateur voit la liste pendant ~5s avant
     que le throttle GPS ne ré-déclenche un recomputeRoute qui ré-affiche
     enfin la card. D'où le délai signalé.
     Avec l'ordre actuel : la card écrase la liste = card visible immédiatement. */
  _updateUberProgress();
  selectBestPatient();
...
function markUberAbsent() {
  const p = APP.get('nextPatient'); if (!p) return;
  p.absent = true;
  // ⚡ Même ordre critique que markUberDone (voir explication ci-dessus)
  _updateUberProgress();
  selectBestPatient();
  if (typeof _updateLiveCADisplay === 'function') _updateLiveCADisplay();
}
